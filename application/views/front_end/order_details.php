...
<html>
    <head>
        ...
        <?php if ($order != NULL) { ?>
            <title><?= $this->config->item('website_name') . ' | ' . $order->title ?></title>
            <meta name="keywords" content="<?= $order->title ?>">
            <meta name="description" content="<?= character_limiter(strip_tags(preg_replace('/(<[^>]+) style=".*?"/i', '$1', $order->description)), 300) ?>">
            <meta property="og:title" content="<?php echo $this->config->item('website_name') . ' | ' . $order->title; ?>" />
            <meta property="og:image" content="<?php echo base_url('uploads/companies/' . $order->manufacturer_image) ?>" />
            <meta property="og:description" content="الرجاء الضغط هنا للإطلاع على كافة تفاصيل الإعلان">
            <meta property="og:site_name" content="<?php echo $this->config->item('website_name'); ?>">
        <?php } else { ?>
            <title><?= $this->config->item('website_name').'| '.'تفاصيل الطلب' ?></title>
        <?php } ?>
            ...
            <div id="contentWrapper">
                <div class="page-title title-1">
                    <div class="container">
                        <div class="row">
                            <div class="cell-12">
                                <h1 class="fx" data-animate="fadeInLeft">تفاصيل<span> الإعلان</span></h1>
                                <div class="breadcrumbs main-bg fx" data-animate="fadeInUp">
                                    <span class="bold">أنت الأن داخل:</span><a href="#">الرئيسية</a><span class="line-separate">/</span><span>تفاصيل طلب الشراء</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="sectionWrapper">
                    <div class="container">
                        <div class="row">
                            <div class="cell-9">
                                <!-- Post Details Start -->
                                <?php $this->load->view('front_end/includes/order_details'); ?>
                                <!-- Post Details End --> 
                            </div>
